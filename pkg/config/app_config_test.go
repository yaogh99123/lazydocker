...
func TestDockerComposeCommandNoFiles(t *testing.T) {
	composeFiles := []string{}
	conf, err := NewAppConfig("name", "version", "commit", "date", "buildSource", false, composeFiles, "projectDir", "", false)
	if err != nil {
...
func TestWritingToConfigFile(t *testing.T) {
	...
	conf, err := NewAppConfig("name", "version", "commit", "date", "buildSource", false, emptyComposeFiles, "projectDir", "", false)
	if err != nil {
		t.Fatalf("Unexpected error: %s", err)
	}
